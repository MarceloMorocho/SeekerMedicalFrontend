...
@Component({
  selector: 'app-registrar-farmacia',
  templateUrl: './registrar-farmacia.page.html',
  styleUrls: ['./registrar-farmacia.page.scss'],
})

export class RegistrarFarmaciaPage implements OnInit {
  ...
  constructor(private navCtrl: NavController, 
    ...
    this.farmacia.farUbicacionLat = this.farUbicacionLat;
    this.farmacia.farUbicacionLon = this.farUbicacionLon;
    this.ordenarDiasAlfabeticamente(); // Ordena los días alfabéticamente
  }
  ...
  async validarCamposObligatorios() {
    ...
    if (this.idnavactual === 5) {
      if ( 
        ...
        this.farmacia.farClave &&
        this.farConfirmarClave &&
        this.farmacia.farClave === this.farConfirmarClave
      ) { 
  ...
  validarRucYExistencia() {
    ...
    const ruc = this.farmacia.farRuc;
    this.http.get<string>(`http://${ipServidor}/farmacias/validaExistenciaFarRuc/${ruc}`, { responseType: 'text' as 'json' })
    .subscribe(response => {
    ...
    this.http.get<string>(`http://${ipServidor}/farmacias/validaRuc/${ruc}`, { responseType: 'text' as 'json' })
    .subscribe(response => {
      this.respuestaBackendRuc = response;
      // Verifica si la respuesta contiene "El RUC no es válido" y muestra un mensaje en consecuencia
  ...
  validarCorreoElec() {
    ...
    const correoElec= this.farmacia.farCorreoElec;
    this.http.get<string>(`http://${ipServidor}/farmacias/validaExistenciaFarCorreoElec/${correoElec}`, { responseType: 'text' as 'json' })
    .subscribe(response => {
    ...
    this.http.get<string>(`http://${ipServidor}/farmacias/validaCorreoElec/${correoElec}`, { responseType: 'text' as 'json' })
    .subscribe(response => {
  ...
  // Método para registrar el logotipo y transformarlo a byte
    ...
    if (files && files.length > 0) {
      ...
      if (selectedFile.type.startsWith('image/')) {
        ...
        this.convertirImagenABytes(selectedFile).then((bytes) => {
          this.farmacia.farLogotipo = Array.from(bytes);
        ...
        this.farmacia.farLogotipo = this.defaultAvatarUrl;
      }
    } else {
  ...
  farUbicacionLat: number = -2.900128;
  farUbicacionLon: number = -79.005896;

  // Llama al método para obtener la ubicación en ionViewDidEnter
  async ionViewDidEnter() {
    if (this.idnavactual === 3 && !this.locationPermissionsRequested) {
    ...
    this.http.get(`${this.apiUrl}?query=${this.buscarLugar}`).subscribe(
      (data: any) => {
        if (data && data.results && data.results.length > 0) {
          ...
          // Actualiza las propiedades de latitud y longitud de la farmacia
  ...
  // Método para ubicarse automáticamente cuando pulsa sobre el botón "Ubicación actual"
  ...
  farClaveInvalido = false; // Inicialmente se considera válida
  validarClave(){
    if (this.farmacia.farClave && this.farmacia.farClave.length < 8) {
      this.farClaveInvalido = true;
    } else {
      this.farClaveInvalido = false;
    }
  }

  // Método para comparar las claves
  clavesDifieren = false;
  farConfirmarClave: string = ''; 
  compararClaves() {
    if (this.farmacia.farClave !== this.farConfirmarClave) {
  ...
  async agregarDiasYHorarios() {
    ...
    this.ordenarDiasAlfabeticamente();
    // Cerrar el modal
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            // Se agrega el método registrarFarmacia() para guardar en la base de datos
  ...
  async registrarFarmacia() {
    // Inicializa la propiedad farLogotipo con la imagen en bytes del avatar predeterminado
    try {
      ...
      if (this.selectedFile) {
        ...
        this.farmacia.farLogotipo = Array.from(new Uint8Array(avatarArrayBuffer));
      }
      ...
      // URL para guardar los datos de la farmacia en el backend
      const farmaciaResponse = await this.http.post<any>(`http://${ipServidor}/farmacias/save`, this.farmacia).toPromise();
      console.log('Respuesta de la API (Farmacia):', farmaciaResponse);
      if (farmaciaResponse === true) {
        console.log('Farmacia registrada con éxito.');
        for (const diaHorario of this.diasYHorariosSeleccionados) {
          ...
          if (diaId !== undefined) {
            ...
            console.error('El diaId es undefined. Verifica la respuesta del backend o la configuración del mapeo de días.');
          }
          if (diaId) {
        ...
        // Después de guardar los horarios
        ...
        if (allDataSaved) {
          ...
          console.error('Error al registrar la farmacia. Verifica la respuesta del backend:', farmaciaResponse.error);
      }
    } catch (error) {
        ...
        if (error instanceof HttpErrorResponse) {
          if (error.status === 422) {
            ...
            console.error('Error al registrar la farmacia:', errorMessage);
            ...
            console.error('Error HTTP al registrar la farmacia:', error.statusText);
          }
        } else {
            console.error('Error desconocido al registrar la farmacia:', error);
        }
    }
  }
